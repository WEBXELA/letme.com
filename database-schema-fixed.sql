-- FIXED Database Schema for LetMe.com
-- This version will definitely work with Supabase
...
DROP POLICY IF EXISTS "Allow public read access to Areas" ON "Areas";
...
DROP POLICY IF EXISTS "Allow admin full access to Properties" ON "Properties";
DROP POLICY IF EXISTS "Allow admin full access to Units" ON "Units";
DROP POLICY IF EXISTS "Allow public to insert applications" ON applications;
DROP POLICY IF EXISTS "Allow admin to read applications" ON applications;
DROP POLICY IF EXISTS "Allow admin to update applications" ON applications;
DROP POLICY IF EXISTS "Allow admin to delete applications" ON applications;
DROP POLICY IF EXISTS "Allow users to add themselves as admin" ON admin_users;
DROP POLICY IF EXISTS "Allow users to read their own admin status" ON admin_users;

-- Create Areas table (with quotes for case sensitivity)
CREATE TABLE "Areas" (
...
-- Create Properties table (with quotes for case sensitivity)
...
-- Create Units table (with quotes for case sensitivity)
CREATE TABLE "Units" (
...
-- Function to help add admin users
